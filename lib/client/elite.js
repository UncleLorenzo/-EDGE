...
(function () {
  ...
  var NAV = [
    ...
    { href: "/tracker.html", label: "Rug Radar" },
    // Token intentionally lives under the "Buy $EDGE" button (nav-right), not the
    // menu — token launch is TBD, so it's not a primary destination yet.
  ];
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", normalizeNav);
  else normalizeNav();
})();
