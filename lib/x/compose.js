...
// 🗣️ A market blowing up in the news/chatter — the "big event + a market" post.
export function composeBuzz(m) {
  return cap(`🗣️ TRENDING\n\n"${mkt(m.title, 90)}" is one of the most-talked-about markets right now — ${m.thread_count || "several"} sources buzzing.\n\nSee where the money's going 👇\n${SITE}/buzz`);
}
